...
function EmotionFXTests:UpdateCamera(deltaTime)
    ...
    relCameraTm:SetTranslation(relCameraTm:GetTranslation() - relCameraTm:GetBasis(1) * self.Properties.CameraDistance + Vector3(0, 0, 1.5));
...
function EmotionFXTests:UpdateLocomotion(deltaTime)
    ...
    if (self.moveLeftRight ~= 0 or self.moveForwardBack ~= 0) then
        ...
        local playerFacing = playerTm:GetBasis(1);
        local currentAngle = math.atan2(-playerFacing.x, playerFacing.y);
        local cameraFacing = cameraTm:GetBasis(1);
        ...
        local currentRotation = playerTm:GetRotation();
